...
namespace LLMR.Services
{
    public class PythonEnvironmentManager : ReactiveObject
    {
        ...
        public PythonEnvironmentManager(string pythonPath)
        {
        ...
        private async Task<bool> CheckPythonEnvironmentAsync()
        {
            try
            {
                string pythonExecutable = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                    ? Path.Combine(_pythonPath, "Scripts", "python.exe")
                    : Path.Combine(_pythonPath, "bin", "python3.12");
                ...
                var startInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = "-c \"import sys; import openai; import gradio; print(sys.version); print(openai.__version__); print(gradio.__version__)\"",
